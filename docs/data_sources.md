...
- **Direct exposure baseline** — built by
  `tools/build_baseline_exposure.py` from the seven food-group and seafood
...
- **Calories** — built by `tools/build_baseline_calories.py` from the public
  GDD-IA 2020 all-food table and WPP weights.
...
1. **IHME GBD raw data are not redistributable.** Only reduced, adapted
   ...
   `tools/build_baseline_exposure.py` and `tools/build_baseline_calories.py`.

...
- **Health and demographic data** — relative-risk curves, mortality, population
  and life tables. Regenerated by `tools/prepare_data.py` from WHO GHE, IHME
  GBD, and UN WPP downloads.
...
- **Sodium mediator baseline** — country × age × sex urinary-sodium and SBP
  ...
  GBD 2023 release. Public sodium/SBP BoP curves are built separately by
  `tools/build_sodium_relative_risks.py`.
...
`tools/prepare_data.py` regenerates these five files:
...
The nutrient and mediator builders also download IHME's public GBD 2021
location hierarchy and use its level-3 location IDs to select national rows.

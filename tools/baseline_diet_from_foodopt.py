...
is currently produced from the sibling ``food-opt`` project's processed dietary
intake (GDD-IA / NHANES, with the unprocessed/processed red-meat split from the
GDD-IA processed fraction). That dataset is openly available during development
but is **not** part of ``mealhealth``'s self-contained pipeline.
...
itself changes. It is intentionally the only tool that references ``food-opt``.

Run with food-opt's environment, e.g.::

    cd /path/to/food-opt
    .pixi/envs/default/bin/python /path/to/mealhealth/tools/baseline_diet_from_foodopt.py
"""
...
# Path to a food-opt checkout (the temporary baseline-diet source). Future:
# a Zenodo DOI / URL once the baseline diet is published.
...
def build_baseline_intake() -> pd.DataFrame:
    ...
    Plant groups and the combined red-meat total come from food-opt's validated
...
def _burden_country_set() -> set[str]:
    """Countries with complete burden inputs in food-opt's processed data.
